...
def main():
    ...
    with timer("tfidf"):
        all_df["author"].fillna("", inplace=True)
        all_df["publisher"].fillna("", inplace=True)
        texts = (
            all_df["title"] + " " + all_df["author"] + " " + " " + all_df["publisher"]
        ).to_list()
        title_tfidf_matrix = TfidfVectorizer(
            ngram_range=(1, 3),
            min_df=3,
            max_df=0.9,
            use_idf=True,
        ...
        tfidf_matrix = svd_.fit_transform(title_tfidf_matrix)
        for i in range(tfidf_matrix.shape[1]):
            df[f"tfidf_{i}"] = tfidf_matrix[: len(df), i]
            test_df[f"tfidf_{i}"] = tfidf_matrix[len(df) :, i]
    ...
    with timer("Target Encoding"):
        ...
        for col_a, col_b in tqdm(combinations(combination_cols, 2)):
        ...
        for col_a in tqdm(combination_cols1):
            for col_b in tqdm(combination_cols2):
                df, test_df = target_encoding(
                    df, test_df, [col_a, col_b], target_col="rating"
                )
